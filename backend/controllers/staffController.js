...
const getAllStaff = async (req, res) => {
  const sql = 'SELECT id, name, username, role, shift, created_at FROM Staff ORDER BY name';
  try {
...
const getStaffByRole = async (req, res) => {
  const sql = 'SELECT id, name, username, role, shift, created_at FROM Staff WHERE role = $1 ORDER BY name';
  try {
...
const getStaffByShift = async (req, res) => {
  const sql = 'SELECT id, name, username, role, shift, created_at FROM Staff WHERE shift = $1 ORDER BY name';
  try {
...
const createStaff = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      ...
      staff: { id: result.rows[0].id, name, username, role, shift }
    });
  } catch (err) {
    if (err.code === '23505') {
...
const updateStaff = async (req, res) => {
  ...
  try {
    ...
    const sql = 'UPDATE Staff SET name = $1, username = $2, role = $3, shift = $4, updated_at = CURRENT_TIMESTAMP WHERE id = $5 RETURNING id';
    const result = await db.query(sql, [name, username, role, shift, req.params.id]);
    if (!result.rows || result.rows.length === 0) return res.status(404).json({ error: 'Staff not found' });
    res.json({ message: 'Staff updated successfully', staff: { id: req.params.id, name, username, role, shift } });
  } catch (err) {
