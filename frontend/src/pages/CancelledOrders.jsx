...
const CancelledOrders = () => {
  ...
  useEffect(() => {
    const filtered = orders.filter((order) => {
      ...
      return (
        ...
        (order.table_no && order.table_no.toString().includes(query)) ||
        (order.waiter_name && order.waiter_name.toLowerCase().includes(query)) ||
        (order.cancellation_reason && order.cancellation_reason.toLowerCase().includes(query)) ||
        (order.cancelled_by_name && order.cancelled_by_name.toLowerCase().includes(query))
      );
    });
  ...
  const fetchOrders = async () => {
    try {
      const response = await orderAPI.getByStatus('cancelled');
      const cancelled = Array.isArray(response.data) ? response.data : [];
      setOrders(cancelled);
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Review all cancelled orders, cancellation reasons, and timestamps.
          </Typography>
        </Box>
        <Button startIcon={<Cancel />} variant="outlined" onClick={fetchOrders}>
          Refresh
        </Button>
      </Box>

      <TextField
        label="Search cancelled orders"
        variant="outlined"
        size="small"
        value={searchTerm}
            ...
            {filteredOrders.length === 0 ? (
              ...
              filteredOrders.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell>{order.cancelled_by_name || '—'}</TableCell>
                  <TableCell>{order.cancelled_at ? new Date(order.cancelled_at).toLocaleString() : '—'}</TableCell>
                    ...
                    {orderItems.length === 0 ? (
                      ...
                      orderItems.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>Rs. {Number(item.unit_price || item.price || 0).toFixed(2)}</TableCell>
                          <TableCell>Rs. {Number((item.quantity || 0) * (item.unit_price || item.price || 0)).toFixed(2)}</TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
